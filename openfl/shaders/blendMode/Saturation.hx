...
/**
 * Replaces the saturation of the base color by the saturation of the blend color.
 * @author hsv convertion from https://newbedev.com/from-rgb-to-hsv-in-opengl-glsl
 * @author Loudo
 */
class Saturation extends GraphicsShader
{
	...
	@:glFragmentSource(
		...
		vec3 blendSaturation(vec3 base, vec3 blend) {
			...
			hsvBase.y = hsvBlend.y;
			return hsv2rgb(hsvBase);
		}

		vec3 blendSaturation(vec3 base, vec3 blend, float opacity) {
			return (blendSaturation(base, blend) + base * (1.0 - opacity));
		}
		...
		void main(void) {
			...
			vec3 blendedColor = blendSaturation(bgColor.rgb, fgColor.rgb, fgColor.a);
